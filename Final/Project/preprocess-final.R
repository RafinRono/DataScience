...
count <- 1
data <- data.frame(Title = character(), Article = character(), Domain = character(), stringsAsFactors = FALSE)

for (i in seq_along(Titles)) {
  full_url <- ifelse(grepl("^https?://", links[i]), links[i], paste0("https://www.thedailystar.net", links[i]))

  article_page <- read_html(full_url)
  
  title <- article_page %>%
    html_nodes("h1.fw-700.e-mb-16.article-title") %>%
    html_text() %>%
    .[1]  
  ...
  result <- paste(as.character(count), "out of", as.character(length(Titles)), '-', title[1])
  count <- count + 1
  print(result)
}
...
write.xlsx(data_processed, "data_processed.xlsx")
